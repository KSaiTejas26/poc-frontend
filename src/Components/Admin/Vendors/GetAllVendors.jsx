...
const GetAllVendors = () => {
  ...
  const handleSelect = (event, id) => {
    if (event.target.checked) {
      setSelectedVendors((prevSelectedVendors) => [...prevSelectedVendors, id]);
    } else {
      setSelectedVendors((prevSelectedVendors) =>
        prevSelectedVendors.filter((vendorId) => vendorId !== id)
      );
    }
  };
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      const response = await axios.delete(`http://localhost:3000/api/admin/vendor/deleteMany`, {
