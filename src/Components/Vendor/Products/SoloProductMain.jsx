...
// export default function SoloProductMain() {
...
export default function SoloProductMain({ productId }) {
  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        const response = await axios.get(`http://localhost:3000/api/vendor/getsoloproduct/6663f33ddae9d350e7ecb8e4`)
        ...
        //   vendor._id.includes(product.vid)
        // );
        //  setFilteredData(filtered);
        // console.log(filteredData.data);
        // Initialize the color and size if they exist
        // if (response.colors) setSelectedColor(response.colors[0])
        // if (response.sizes) setSelectedSize(response.sizes.find(size => size.inStock))
      } catch (err) {
                ...
                <div className="flex items-center">
                  {[0, 1, 2, 3, 4].map((rating) => (
                    <StarIcon
                      key={rating}
                      className={classNames(
                        product.rating > rating ? 'text-amber-200' : 'text-slate-200',
                        'h-5 w-5 flex-shrink-0'
                      )}
                      aria-hidden="true"
                    />
          ...
          <div className="py-10 lg:col-span-2 lg:col-start-1 lg:border-r lg:border-gray-200 lg:pb-16 lg:pr-8 lg:pt-6">
            ...
            {/* <div className="mt-10">
              ...
              <div className="mt-4 space-y-6">
                ...
                <p className="text-sm text-gray-600">{product.productDetails.dimensions}</p>
                <p className="text-sm text-gray-600">{product.productDetails.designerName}</p>
                <p className="text-sm text-gray-600">{product.productDetails.countryOfOrigin}</p>
              </div>
            </div> */}
          </div>
        </div>
